...
import ro.mentenantapc.commons.helpers.MyContentProvider.PERMISSION_WRITE_GLOBAL_SETTINGS
import ro.mentenantapc.commons.models.AlarmSound
import ro.mentenantapc.commons.models.BlockedNumber
import java.io.File
...
fun Context.isAProApp() = packageName.startsWith("ro.mentenantapc.") && packageName.removeSuffix(".debug").endsWith(".pro")
...
fun Context.getCanAppBeUpgraded() = proPackages.contains(baseConfig.appId.removeSuffix(".debug").removePrefix("ro.mentenantapc."))
...
fun Context.isDefaultDialer(): Boolean {
    return if (!packageName.startsWith("ro.mentenantapc.contacts") && !packageName.startsWith("ro.mentenantapc.phone")) {
        true
    } else if ((packageName.startsWith("ro.mentenantapc.contacts") || packageName.startsWith("ro.mentenantapc.phone")) && isQPlus()) {
