...
import ro.mentenantapc.commons.compose.extensions.DEVELOPER_PLAY_STORE_URL
import ro.mentenantapc.commons.databinding.DialogTitleBinding
import ro.mentenantapc.commons.dialogs.*
import ro.mentenantapc.commons.dialogs.WritePermissionDialog.WritePermissionDialogMode
import ro.mentenantapc.commons.helpers.*
import ro.mentenantapc.commons.helpers.MyContentProvider.COL_LAST_UPDATED_TS
import ro.mentenantapc.commons.helpers.MyContentProvider.MY_CONTENT_URI
import ro.mentenantapc.commons.models.*
import ro.mentenantapc.commons.views.MyTextView
import java.io.*
...
fun Activity.launchPurchaseThankYouIntent() {
    hideKeyboard()
    try {
        launchViewIntent("market://details?id=ro.mentenantapc.thankyou")
...
fun Activity.openPathIntent(
    ...
    ensureBackgroundThread {
        ...
        Intent().apply {
            ...
            if (applicationId == "ro.mentenantapc.gallery" || applicationId == "ro.mentenantapc.gallery.debug") {
...
fun BaseSimpleActivity.launchCallIntent(recipient: String, handle: PhoneAccountHandle? = null) {
    handlePermission(PERMISSION_CALL_PHONE) {
        ...
        Intent(action).apply {
            ...
            if (isDefaultDialer()) {
                val packageName = if (baseConfig.appId.contains(".debug", true)) "ro.mentenantapc.phone.debug" else "ro.mentenantapc.phone"
                val className = "ro.mentenantapc.phone.activities.DialerActivity"
